...
void show_progress_bar(const long long file_size, const long long cur_size){
    static chrono::steady_clock::time_point last_clk, last_update_byte_rate_time;
    static int last_size ,br_last_size;
    static double last_byte_rate;
    ...
    double byte_rate;
    double time_diff, update_byte_rate_time_diff;
    if(cur_size == 0){
        last_size = cur_size;
        last_clk = now_clk;
        last_update_byte_rate_time = now_clk;
        ...
        update_byte_rate_time_diff = chrono::duration_cast<chrono::milliseconds>((now_clk-last_update_byte_rate_time)).count()/1000.0;
        if(time_diff < 0.01 && file_size>cur_size) return;
        else{
            progress_rate = cur_size*1.0/file_size;
            if(update_byte_rate_time_diff > 0.3){
                byte_rate = 1.0*(cur_size-br_last_size)/update_byte_rate_time_diff;
                last_byte_rate = byte_rate;
                br_last_size = cur_size;
                last_update_byte_rate_time = now_clk;
            }
            else{
                byte_rate = last_byte_rate;
            }
            last_clk = now_clk;
            last_size = cur_size;
...
string time_format(long long t){
    string u = to_string((t/1000000)/1000.0);
    return u.substr(0, min(u.find('.')+4, u.size())) + " s";
}
...
int main(){
    ...
    try{
        ...
        cout << "complete! (" << time_format((chrono::steady_clock::now() - time_stamp).count()) << ")" << endl;
    }
    catch(const std::exception& e){
        cout << "failed!" << endl;
        cerr << e.what() << endl;
    ...
    if(is_sender){
        ...
        cout << "complete! You are receiver!" << endl;
        cout << "Waiting sender selecting and processing file...";
        cout.flush();
    }
    ...
    if(is_sender){
        ...
        try{
            ...
            socket.wait(socket.wait_read);
            socket.read_some(asio::buffer(rcv_buf));
            if(rcv_buf.data() != ACK) exception_exit("sending file info to rcver, replying other than ACK", socket);
        }
        catch(const std::exception &e){
            cerr << e.what() << endl;
    ...
    if(is_sender){
        ...
        try{
            ...
            socket.close();
            cout << "file delivered successully! (" << time_format((chrono::steady_clock::now() - time_stamp).count()) << ")" << endl;
        }
        catch(const std::exception &e){
            cerr << e.what() << endl;
    ...
    cout << "Mission complete, thank you for using!" << endl;
}